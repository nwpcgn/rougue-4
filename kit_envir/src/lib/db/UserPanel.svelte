...
{#snippet userPanel()}
	<fieldset class="fieldset bg-base-100 border-base-300 rounded-box heading-2 w-xs px-8 py-6">
		<legend class="fieldset-legend animate-infinite animate-pulse">
			<span class="text-2xl font-bold">User Panel</span>
		</legend>
		<div class="animate-delay-[2200] animate-duration-[800ms] animate-fade animate-once grid">
			<label class="label">Uid</label>
			<input value={auth.userId()} type="text" class="input input-md" readonly />
			<label class="label">Email</label>
			<input value={auth.userMail()} type="email" class="input input-md" readonly />
			<label class="label">Username</label>
			<input value={auth.user?.user_metadata?.name} type="text" class="input input-md" />
